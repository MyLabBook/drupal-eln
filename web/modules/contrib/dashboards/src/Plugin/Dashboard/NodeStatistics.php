...
class NodeStatistics extends DashboardBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, CacheBackendInterface $cache, QueryFactory $query_factory, EntityTypeBundleInfoInterface $entity_type_info) {
  ...
  public function buildRenderArray($configuration): array {
    ...
    foreach ($types as $bundle => $type) {
      $data = $this->entityQuery->get('node')->condition('type', $bundle)->count()->execute();
      $rows[] = [
        $type['label'],
        $data,
      ];
